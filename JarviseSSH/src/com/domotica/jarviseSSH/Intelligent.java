...
//					String c[]=texts.getFirst().split("\\s",3);
	...
	if(texts.getFirst().matches(WORD_CONSTANTS.orario)){
			...
			if(testo.matches(WORD_CONSTANTS.sbagliato)){
				...
				if(state==1)new MultiThread("127.0.0.1", 9001,new PaccoString((texts.getFirst().split("\\s",2)[1])));
				if(state==2)new MultiThread("127.0.0.1", 9001,new PaccoString((texts.getFirst().split("\\s",4)[3])));
				if(state==3)new MultiThread("127.0.0.1", 9001,new PaccoStringOn("Accensione: "+(texts.getFirst().split("\\s",2)[1])));
				if(state==4)new MultiThread("127.0.0.1", 9001,new PaccoStringOff("Spegnimento: "+(texts.getFirst().split("\\s",2)[1])));
				state = 0 ;
				while(MainActivity.talker.isSpeaking());
				fine = true;
				return;
